 ...
 * Version: 1.2.0 (Data Guard Update)
 * Date: December 19, 2025
 * * CHANGE LOG:
 * - v1.2.0: Added "Data Guard" validation logic. 
 * - Implemented `validateFile` to check for specific column headers (Priority, SamAccountName, etc.).
 * - Added UI feedback (Green Check / Red Error) to upload cards.
 * - Prevents processing if files are empty or malformed to avoid false "Clean Health" reports due to bad data.
 * - v1.1.0: Fixed PDF/Print export bugs and JSX syntax errors.
...
const validateFile = (type, parsed) => {
    ...
    // Define the specific "fingerprint" of columns required for each file type
    if (type === 'sql') required = ['priority', 'finding'];
    if (type === 'ad') required = ['groupname', 'samaccountname'];
    if (type === 'backup') required = ['message']; // 'timecreated' is often variable, we'll check loosely

    const missing = required.filter(r => !headers.includes(r));
    
    if (missing.length > 0) {
        return { valid: false, error: `Missing columns: ${missing.join(', ')}. Wrong file?` };
    }
